// --- DATA ---
// IMAGE FIX: All queen image URLs have been individually sourced from the Fandom wiki's static image host and verified to be working.
...
const queens = [
    ...
    // S3 with Images
    { id: 'maxie', name: 'Maxie', season: 3, image: 'https://static.wikia.nocookie.net/logosrupaulsdragrace/images/8/85/MaxieDRPH3MiniPromo.jpg', stats: { runway: 9, comedy: 10, acting: 9, dance: 7, design: 8, lipsync: 6 } },
...
function promptForWinner(scores) {
    ...
    topQueens.forEach(s => { 
        html += `<button class="secondary-button w-full p-4 rounded-lg text-left flex items-center gap-4" data-winner-id="${s.queen.id}">
                    <img src="${s.queen.image}" class="w-16 h-16 rounded-full object-cover">
                    <div class="flex-1">
    ...
    resultsContainer.querySelectorAll('button').forEach(btn => btn.addEventListener('click', (e) => {
        ...
        setTimeout(() => handleWinnerSelection(btn.dataset.winnerId, scores), 500);
    })); 
...
function promptForBottoms(scores) {
    ...
    resultsContainer.querySelectorAll('button').forEach(btn => btn.addEventListener('click', (e) => {
        e.currentTarget.classList.add('selected');
        resultsContainer.querySelectorAll('button').forEach(b => b.disabled = true);
        setTimeout(() => handleBottomSelection(btn.dataset.safeId, scores), 500);
    })); 
}
...
function assignPlacements(results) { const len = results.length; if (len === 0) return []; return results.map((r, i) => { let placement; if (i === 0) placement = 'WIN'; else if (len > 5 && i <= 2) placement = 'HIGH'; else if (i >= len - 2) placement = 'BTM'; else if (len > 4 && i === len - 3) placement = 'LOW'; else placement = 'SAFE'; return { queen: r.queen, placement: placement }; }); }
function displayPlacements(placements) {
    ...
    placements.sort((a, b) => { const order = { 'WIN': 0, 'HIGH': 1, 'SAFE': 2, 'LOW': 3, 'BTM': 4, 'BTM2': 4, 'ELIM': 5 }; return order[a.placement] - order[b.placement]; }).forEach(({queen, placement}) => { 
        ...
        html += `<div class="bg-black/50 p-3 rounded-lg text-center ${winClass} transition-all duration-500">
                    <img src="${queen.image}" class="w-24 h-24 rounded-full mx-auto border-4 border-gray-600 object-cover ${winImgClass}">
...
function displayTrackRecord() {
    ...
    let maxEpisodes = 0;
    fullCast.forEach(q => { if(q.trackRecord.length > maxEpisodes) maxEpisodes = q.trackRecord.length; });
...
function displayTop2Results(top2, eliminated) { 
    resultsContainer.innerHTML = `<div class="text-center space-y-4 max-w-2xl mx-auto">
        ...
        <div class="p-4 rounded-lg bg-green-900/50 text-3xl font-display tracking-widest flex items-center justify-center gap-4">
            <img src="${top2[0].queen.image}" class="w-16 h-16 rounded-full object-cover">${top2[0].queen.name}
        </div>
        <div class="p-4 rounded-lg bg-green-900/50 text-3xl font-display tracking-widest flex items-center justify-center gap-4">
            <img src="${top2[1].queen.image}" class="w-16 h-16 rounded-full object-cover">${top2[1].queen.name}
        </div>
...
function promptForWinnerCrown(q1, q2, score1, score2){ 
    ...
    const createButton = (q, score) => `<button class="secondary-button w-full p-4 rounded-lg flex items-center gap-4" data-winner-id="${q.id}">
        <img src="${q.image}" class="w-24 h-24 rounded-full object-cover">
...
function displayWinner(winner, runnerUp) {
    ...
    resultsContainer.innerHTML = `<div class="text-center space-y-4 max-w-2xl mx-auto">
        ...
        <div class="py-8">
            <img src="${winner.image}" class="w-48 h-48 rounded-full object-cover mx-auto border-8 border-amber-400 placement-WIN-img">
...
function updateSelectionUI() { 
    ...
    selectedCast.forEach(q => { 
        ...
        li.innerHTML = `<img src="${q.image}" class="w-8 h-8 rounded-full mr-3 object-cover"><span class="font-bold text-sm">${q.name}</span>`; 
...
function toggleQueenSelection(queen) { const i = selectedCast.findIndex(q => q.id === queen.id); if (i > -1) selectedCast.splice(i, 1); else if (selectedCast.length < MAX_CAST_SIZE) selectedCast.push(queen); updateSelectionUI(); }
...
window.addEventListener('load', () => {
    ...
    sortedQueens.forEach(q => {
        ...
        card.innerHTML = `<img src="${q.image}" class="w-full aspect-square rounded-md object-cover"><p class="mt-2 text-xs font-bold truncate">${q.name}</p>`; 
